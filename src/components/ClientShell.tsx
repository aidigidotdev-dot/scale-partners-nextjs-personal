  ...
  return (
    <div className="flex flex-col min-h-screen bg-gold-50/10 selection:bg-gold-200 text-zinc-900 relative">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[-15%] left-[-15%] w-[65%] h-[65%] rounded-full bg-[#00d4aa]/[0.04] blur-[140px]"></div>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        {modalOpen && (
          <div id="corporate_modal_backdrop" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/75 backdrop-blur-md animate-fade-in">
            <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-[0_30px_100px_rgba(15,23,42,0.22)] overflow-hidden border border-zinc-150 max-h-[90vh] overflow-y-auto">
              
              {/* Modal Exit handle */}
              <button
                id="modal_dismiss_trigger"
                onClick={() => setModalOpen(false)}
                className="absolute top-4 right-4 z-10 p-2.5 bg-zinc-50 hover:bg-zinc-100 rounded-full text-zinc-500 hover:text-[#00d4aa] transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
